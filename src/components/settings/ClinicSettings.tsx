...
export const ClinicSettings: React.FC<ClinicSettingsProps> = ({
  ...
  const fetchWaWebStatus = async () => {
    const res = await getWhatsAppWebStatus();
    setWaWebStatus(res);
  };
      ...
      {activeSubTab === 'whatsapp' && whatsapp && (
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-br from-emerald-900 to-slate-900 text-white rounded-3xl p-6 shadow-lg border border-emerald-800/50 space-y-5 text-xs">
            ...
            ) : waWebStatus.qrDataUrl || waWebStatus.status === 'qr_ready' ? (
              <div className="bg-slate-950/80 border border-emerald-800/60 rounded-2xl p-6 text-center max-w-md mx-auto space-y-4">
                <h4 className="font-extrabold text-white text-sm">Scan QR Code with WhatsApp</h4>
                <div className="bg-white p-3 rounded-2xl inline-block shadow-xl border-4 border-emerald-500/40">
                  <img
                    src={waWebStatus.qrDataUrl || 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="200" height="200" viewBox="0 0 200 200"><rect width="200" height="200" fill="%23ffffff"/><text x="100" y="105" font-family="sans-serif" font-size="14" text-anchor="middle" fill="%230f172a">Generating QR...</text></svg>'}
                ...
                <div className="flex flex-wrap items-center justify-center gap-3 pt-1">
                  ...
                  >
                    ...
                    <span>Simulate Device Link</span>
                  </button>
                </div>
              </div>
            ) : (
